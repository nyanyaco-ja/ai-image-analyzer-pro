...
class ModernImageAnalyzerGUI:
    ...
    def extract_hallucination_suspects(self):
        ...
        try:
            ...
            # ハルシネーション検出ロジック（2つの方法を併用）

            # === 方法A: 固定閾値（絶対値基準・保守的） ===
            # パターン1A: SSIM高 × PSNR低（固定値）
            hallucination_1a_fixed = df[(df['ssim'] > 0.97) & (df['psnr'] < 25)]

            # === 方法B: 動的閾値（データ相対基準・感度高） ===
            # パターン1B: SSIM高 × PSNR低（quantile）
            ssim_high = df['ssim'].quantile(0.75)  # 上位25%
            psnr_low = df['psnr'].quantile(0.25)   # 下位25%
            ...
            # 全パターン結合（重複除去）
            hallucination_all = pd.concat([hallucination_1, hallucination_2, hallucination_3]).drop_duplicates()

            # モデル別集計
            model_counts = hallucination_all['model'].value_counts()

            # パターン別集計（サブパターン含む）
            pattern1a_count = len(hallucination_1a_fixed)  # 固定閾値のみ
            pattern1b_count = len(hallucination_1b_quantile)  # quantileのみ
            pattern1_count = len(hallucination_1)  # 統合後
            pattern2_count = len(hallucination_2)
            pattern3_count = len(hallucination_3)
            ...
            # 結果表示
            result_text = f"={'='*60}\n"
            result_text += f"ハルシネーション疑いデータ分析結果\n"
            result_text += f"{'='*60}\n\n"

            result_text += f"総データ数: {len(df)}件\n"
            result_text += f"ハルシネーション疑い: {len(hallucination_all)}件 ({len(hallucination_all)/len(df)*100:.1f}%)\n\n"

            result_text += f"【パターン別検出数】\n"
            result_text += f"  パターン1 (SSIM高×PSNR低): {pattern1_count}件\n"
            result_text += f"    - 固定閾値 (SSIM>0.97 & PSNR<25): {pattern1a_count}件\n"
            result_text += f"    - 動的閾値 (quantile 75%/25%): {pattern1b_count}件\n"
            result_text += f"      閾値: SSIM>={ssim_high:.3f}, PSNR<={psnr_low:.1f}\n"
            result_text += f"  パターン2 (シャープネス高×ノイズ高): {pattern2_count}件\n"
            result_text += f"    - 閾値: シャープネス>{sharpness_75:.1f}, ノイズ>{noise_75:.4f}\n"
            result_text += f"  パターン3 (エッジ密度異常×低品質): {pattern3_count}件\n"
            result_text += f"    - 閾値: エッジ密度>{edge_90:.4f}, 局所品質<{quality_25:.4f}\n"
            result_text += f"  ※重複を除いた合計: {len(hallucination_all)}件\n\n"

            result_text += f"【モデル別】\n"
            for model in sorted(model_counts.index):
                count = model_counts[model]
                percentage = count / len(df) * 100
                result_text += f"  {model}: {count}件 ({percentage:.1f}%)\n"
            ...
            pattern_counts_detailed = [pattern1a_count, pattern1b_count, pattern2_count, pattern3_count]
            colors = ['#FF6B6B', '#FF9999', '#4ECDC4', '#45B7D1']

            # 0件のパターンを除外
            filtered_labels = []
            filtered_counts = []
            filtered_colors = []
            for label, count, color in zip(pattern_labels, pattern_counts_detailed, colors):
                if count > 0:
            ...
            ax2.set_title('パターン別検出数\nP1固定=SSIM>0.97&PSNR<25\nP1動的=quantile', fontsize=11, fontweight='bold')
            ...
            plt.suptitle(f'ハルシネーション疑いデータ分析 (n={len(hallucination_all)})',
