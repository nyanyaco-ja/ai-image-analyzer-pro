...
- **Zenodo (DOI: 10.5281/zenodo.17470655)**: 永久保存版データアーカイブ
...
元画像とAI処理画像のサイズが異なる場合でも、**自動的にサイズを調整して分析**できます：

**機能概要:**
- **自動サイズ検出**: 両画像のサイズを自動的に検出
- **スマートリサイズ**: サイズが異なる場合、AI処理画像を元画像のサイズに自動調整
- **高品質補間**: LANCZOS4補間法により、リサイズによる品質劣化を最小化
- **情報表示**: サイズ不一致時には調整内容を自動通知

**対応ケース:**
- 超解像処理（例: 1000px → 2000px → 1000pxに調整して比較）
- ダウンスケール評価（例: 2000px → 1000px → 2000pxに調整して比較）
- 任意のサイズ組み合わせ（自動調整で常に同じサイズで分析）

**注意事項:**
- 元画像がAI処理画像より大きい場合、AI処理画像をアップサンプリング
- より正確な評価のため、事前にサイズを揃えることを推奨
- バッチ処理モードでも全画像で自動適用
...
### v1.7.0（現在）: 完全国際化対応
- 🌐 **完全国際化対応（i18n）**
  - 860翻訳キーによる日本語/英語完全対応
  - 全UI、ログ、統計分析メッセージの多言語化
  - locales/en.json、ja.jsonによる翻訳管理
  - 動的言語切り替え機能
- 📊 **学術論文形式プロット改善**
  - プロットタイトルを下部配置に統一
  - X軸ラベルを水平表示に変更（視認性向上）
  - サブプロットタイトルの配置最適化
- 🐍 **Python 3.12.10対応**
  - 動作確認環境をPython 3.12.10に更新
  - 詳細インストール手順追加（公式サイト/wingetコマンド）
  - Windows仮想環境セットアップガイド追加
  - `py`/`python`コマンド両対応の説明
  - 管理者権限なし環境への対応（--userフラグ）
- 🏗️ **コードリファクタリング**
  - Mixinパターンによるモジュール分割（modern_gui.py 3,521行削減）
  - コード保守性の向上
- 📈 **統計分析強化**
  - BBI（Boundary Bias Index）分析の可視化改善
  - 27種類の学術プロット自動生成

### v1.6.1: P6構造的欠陥の法則の確立
- 🚀 **核心的発見: P6構造的欠陥の法則の確立**
